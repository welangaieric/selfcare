...
export default function ForgotPasswordPage() {
  ...
  const handleRequestReset = async (e) => {
    ...
    try {
      const response = await fetch("https://server.konnektsmartlife.com/web/forgot-password", {
        method: "POST",
        headers: {
  ...
  const handleVerifyAndReset = async (e) => {
    ...
    try {
      const response = await fetch("https://server.konnektsmartlife.com/web/confirm-forgot-password", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            {step === "request"
              ? "Enter your account and new password to reset"
              : "Enter the verification code to confirm password reset"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {step === "request" ? (
